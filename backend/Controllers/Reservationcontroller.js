...
// Helper: emit real-time notification to a user
const notifyUser = (userId, type, data = {}) => {
  try {
    ...
    if (socketId) io.to(socketId).emit("notification:new", { type, ...data });
  } catch {}
};
...
const createReservation = async (req, res) => {
  try {
    ...
    if (Number(seatsRequested) > trip.availableSeats)
      return res.status(400).json({ message: `Seulement ${trip.availableSeats} place(s) disponible(s)` });
    const existing = await reservationModel.findOne({ trip: tripId, passenger: req.userId, status: { $in: ["pending","accepted"] } });
    if (existing) return res.status(409).json({ message: "Vous avez déjà une réservation pour ce trajet" });

    const reservation = await reservationModel.create({
      trip: tripId, passenger: req.userId,
      seatsRequested: Number(seatsRequested), message: message || "",
      status: "pending", driverRead: false,
    });

    const passenger = await require("../Models/userModel").findById(req.userId).select("firstName lastName");
    notifyUser(trip.driver._id, "reservation", { tripId });
    if (trip.driver?.email) {
      await sendTripEmail(trip.driver.email, trip.driver.firstName, "new_reservation", {
        passengerName: `${passenger.firstName} ${passenger.lastName}`,
        departure: trip.departure, destination: trip.destination, date: trip.date, seats: seatsRequested,
      });
    }

    const populated = await reservationModel.findById(reservation._id)
...
const createStripeSession = async (req, res) => {
  try {
    const reservation = await reservationModel.findById(req.params.id).populate("trip");
    if (!reservation) return res.status(404).json({ message: "Réservation introuvable" });
    if (reservation.status !== "accepted") return res.status(400).json({ message: "Réservation non acceptée" });
    if (reservation.paymentStatus === "paid") return res.status(400).json({ message: "Déjà payé" });
...
const getPassengerReservations = async (req, res) => {
  try {
    const reservations = await reservationModel.find({ passenger: req.userId })
      .populate({ path: "trip", select: "departure destination date time price carImage licensePlate status",
...
const updateReservationStatus = async (req, res) => {
  try {
    ...
    const reservation = await reservationModel.findById(req.params.id)
      .populate("trip").populate("passenger", "email firstName lastName");
    if (!reservation) return res.status(404).json({ message: "Réservation introuvable" });
    ...
    reservation.status = status; reservation.driverRead = true;
    await reservation.save();

    notifyUser(reservation.passenger._id, status === "accepted" ? "accepted" : "rejected");
    if (reservation.passenger?.email) {
      await sendTripEmail(reservation.passenger.email, reservation.passenger.firstName,
        status === "accepted" ? "reservation_accepted" : "reservation_rejected",
        { departure: reservation.trip.departure, destination: reservation.trip.destination,
          date: reservation.trip.date, time: reservation.trip.time });
    }
...
const cancelReservation = async (req, res) => {
  try {
    const reservation = await reservationModel.findById(req.params.id)
      .populate("trip").populate("passenger", "firstName lastName");
    if (!reservation) return res.status(404).json({ message: "Réservation introuvable" });
    ...
    await reservation.save();
    notifyUser(reservation.trip.driver, "cancelled");
...
const getUnreadCount = async (req, res) => {
  try {
    const myTrips = await tripModel.find({ driver: req.userId }).select("_id");
    const tripIds = myTrips.map(t => t._id);
    const count = await reservationModel.countDocuments({ trip: { $in: tripIds }, status: "pending", driverRead: false });
...
const confirmPayment = async (req, res) => {
  try {
    ...
    const reservation = await reservationModel.findById(req.params.id)
      .populate("trip", "departure destination date time price driver")
    ...
    await reservation.save();
    notifyUser(reservation.trip.driver, "payment");
...
module.exports = {
  protect, createReservation, getDriverReservations, getPassengerReservations,
  updateReservationStatus, cancelReservation, getUnreadCount, markAsRead,
  confirmPayment, createStripeSession,
};